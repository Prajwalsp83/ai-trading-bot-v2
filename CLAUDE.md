...
| Date | Phase | Decision |
|---|---|---|
| 2026-06-13 | I.5 | Re-tuned the live ML filter from threshold 0.74 (target-wr 0.60) to **0.40 (target-wr 0.46)** via `train_meta_v2 --force-replace --target-wr 0.46` on the VPS, SMC restarted. Reason: the 0.74 setting over-filtered -- threshold table showed total expected profit (n_kept x expectancy_R) PEAKS around threshold 0.40 (~115 trades, 47% WR, ~29R) and is LOWEST at 0.74 (~20 trades, 60% WR, ~12R) and at no-filter (~255 trades, 41% WR, ~23R). So a MODERATE filter beats both extremes: ~6x more trades than 0.74 + better WR than baseline + most total profit. Based on one 4-mo val window -> monitor live, re-tune is a 30s retrain. Prior model backed up to `meta_labeler.prev.pkl`. shadow stays False. |
